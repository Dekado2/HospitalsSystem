...
public class Panel_addHospitalization extends JPanel {
	/**
	 * 
	 */
	...
	SubPanel_addHospitalization subAdd;
	...
	private JPanel depIDP;
	private JPanel RoomNumP;
	private JPanel panelDepName;
	private JPanel panelHospName;
	 ...
	 * Create the panel.
	 */
	public Panel_addHospitalization(MainMenu mm) {
		...
		JLabel lblNewLabel = new JLabel("Add Hospitalization");
		...
		tfDepName.setFont(new Font("Arial", Font.PLAIN, 14));
		...
		lblDepartmentId.setBounds(-35, 5, 131, 22);
		...
		tfHospName.setFont(new Font("Arial", Font.PLAIN, 14));
		...
		lblHospitalId.setBounds(-55, 5, 106, 22);
		...
		proceedBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				hospitalID = cbHospitalID.getSelectedItem().toString();
				departmentID = cbDepartmentID.getSelectedItem().toString();
				subAdd= new SubPanel_addHospitalization( mm,Panel_addHospitalization.this, patientID ,hospitalID , departmentID, roomNumber);
				mm.getContentPane().add(subAdd);
				Panel_addHospitalization.this.setVisible(false);
				Panel_addHospitalization.this.setEnabled(false);
				subAdd.setBounds(261, 156, 765, 414);
				subAdd.setVisible(true);
				subAdd.setLayout(null);		
			}
		});
		...
		proceedBtn.setIcon(new ImageIcon(Panel_addHospitalization.class.getResource("/view/icons/forward-arrow.png")));
		...
		if (mm.getUser().getType().equals("doctor"))
		{
			cbDepartmentName.addPropertyChangeListener(new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent arg0) {
	...
	if(patientID!=null && hospitalID!=null && departmentID!=null && roomNumber!=null && events!=null)
	fetch();
	...
	public void fetch() {
		...
		cbHospitalID.setSelectedItem(hospitalID);
		cbDepartmentID.setSelectedItem(departmentID);
		departments=Logic.getInstance().getDepartmentsOfHospital(cbHospitalID.getSelectedItem().toString(), Htemp);
		cbRoomNum.setSelectedItem(roomNumber);
		events=Logic.getInstance().getMedicalEvents(tfDepName.getText());
		patients = Logic.getInstance().getPatients();
		for (int i=0; i<patients.size();i++) {
			cbID.insertItemAt(patients.get(i).getId(), i);
		}
	...
	public SubPanel_addHospitalization getSubAdd() {
		return subAdd;
	}
	...
	public JComboBox<String> getCbDepartmentName() {
		return cbDepartmentName;
	}
	public void setCbDepartmentName(JComboBox<String> cbDepartmentName) {
