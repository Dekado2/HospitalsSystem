	...
	public Panel_add_CheckedBy(MainMenu mm, String patientID,String hospitalId,String hospName,String departName,String eventDesc) {
		...
		/*buttonAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				int n = JOptionPane.showOptionDialog(null, "Are you sure you want to add check-up?","Add Check-Up",JOptionPane.YES_NO_OPTION,JOptionPane.INFORMATION_MESSAGE,null,options,options[1]);
				if (n==0)
				{
					try {
					boolean exists = Logic.getInstance().CheckedByExists(cbID.getSelectedItem().toString(),eventCode,tfDoctorID.getText(),shiftNumber);
					int bloodPress1=Integer.valueOf(tfBloodPres1.getText());
					int bloodPress2=Integer.valueOf(tfBloodPres2.getText());
					else if (Calendar.getInstance().getTime().before(checkDate.getDate()))
						JOptionPane.showMessageDialog(null, "Invalid Date!");
					
					else if (cbID.getSelectedItem().toString().length()!=9)
						JOptionPane.showMessageDialog(null, "Invalid ID format! Please input 9 numbers");
					else if (.length()<9 || tfBloodPres2.getText().length()>10)
					...
					else if (cbIsManager.isSelected())
					{
						...
						{
							    Date bday =  new java.sql.Date(checkDate.getDate().getTime());
							if (radioButtonFemale.isSelected())
							Logic.getInstance().AddPerson(tfID.getText(), tfFirstName.getText(), tfSurName.getText(),
									bday , tfCity.getText(), tfStreet.getText(), "F" , tfBloodPres2.getText(), cbBloodType.getSelectedItem().toString(), cbCareFacility.getSelectedItem().toString());
							else
								Logic.getInstance().AddPerson(tfID.getText(), tfFirstName.getText(), tfSurName.getText(),
										bday , tfCity.getText(), tfStreet.getText(), "M" , tfBloodPres2.getText(), cbBloodType.getSelectedItem().toString(), cbCareFacility.getSelectedItem().toString());
							    ...
							    tfCity.setText("");
							    tfStreet.setText("");
							    ((JTextField)checkDate.getDateEditor().getUiComponent()).setText("");
							    ((JTextField)graduationDate.getDateEditor().getUiComponent()).setText("");
							    tfBloodPres2.setText("");
